...
public class Storage {
  ...
  public static void setup() {
    BlockgameEnhanced.getConfig().getStorageConfig().backendType.load();
  ...
  public static void save(@Nullable BlockgameData data) {
    if (data == null) {
      BlockgameEnhanced.LOGGER.warn("BlockgameData is null, not saving");
      return;
    }

    boolean saved = backend.save(data);
    if (!saved) {
      BlockgameEnhanced.LOGGER.error("Failed to save BlockgameData");
    }
  }
}
